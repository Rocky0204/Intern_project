...
def create_route(route: RouteCreate, db: Session = Depends(get_db)):
    ...
    db_operator = db.query(Operator).filter(Operator.operator_id == route.operator_id).first()
    if not db_operator:
...
def update_route(
    ...
    if 'operator_id' in update_data:
        db_operator = db.query(Operator).filter(Operator.operator_id == update_data['operator_id']).first()
        if not db_operator:
            raise HTTPException(status_code=400, detail="Operator not found")
...
def delete_route(route_id: int, db: Session = Depends(get_db)):
    ...
    has_definitions = db.query(RouteDefinition).filter(RouteDefinition.route_id == route_id).first() is not None
    has_journey_patterns = db.query(JourneyPattern).filter(JourneyPattern.route_id == route_id).first() is not None
