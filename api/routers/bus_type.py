...
def create_bus_type(bus_type: BusTypeCreate, db: Session = Depends(get_db)):
    ...
    if existing_type:
        raise HTTPException(
            status_code=400,
            detail="Bus type with this name already exists"
        )
...
def update_bus_type(
    ...
    if 'name' in update_data:
        existing = db.query(BusType).filter(
            BusType.name == update_data['name'],
            BusType.type_id != type_id
        ).first()
        if existing:
            raise HTTPException(
                status_code=400,
...
def delete_bus_type(type_id: int, db: Session = Depends(get_db)):
    ...
    has_blocks = db.query(Block).filter(Block.bus_type_id == type_id).first() is not None
