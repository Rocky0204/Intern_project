...
from ..schemas import StopActivityCreate, StopActivityRead, StopActivityUpdate
...
def read_stop_activities(
    vj_id: int = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
...
def update_stop_activity(
    activity_id: int, activity: StopActivityUpdate, db: Session = Depends(get_db)
):
    db_activity = (
        db.query(StopActivity).filter(StopActivity.activity_id == activity_id).first()
    )
    ...
    if activity.atco_code is not None:
        db_activity.stop_point_id = (
            activity.atco_code
        )  # Map schema's 'atco_code' to model's 'stop_point_id'
