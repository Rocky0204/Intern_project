...
def create_block(block: BlockCreate, db: Session = Depends(get_db)):
    ...
    operator = db.query(Operator).filter(Operator.operator_id == block.operator_id).first()
    if not operator:
        raise HTTPException(
...
def update_block(
    block_id: int, block: BlockUpdate, db: Session = Depends(get_db)
):
    """
    ...
    if "bus_type_id" in update_data:
        bus_type = db.query(BusType).filter(BusType.type_id == update_data["bus_type_id"]).first()
        if not bus_type:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
