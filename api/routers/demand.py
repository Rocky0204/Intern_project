...
from ..models import Demand, StopArea # Import Demand and StopArea for foreign key validation
...
def create_demand(demand: DemandCreate, db: Session = Depends(get_db)):
    ...
    origin_stop_area = db.query(StopArea).filter(StopArea.stop_area_code == demand.origin).first()
    ...
    destination_stop_area = db.query(StopArea).filter(StopArea.stop_area_code == demand.destination).first()
...
@router.get("/{origin}/{destination}/{start_time}/{end_time}", response_model=DemandRead)
def read_demand(
    origin: int,
    destination: int,
    ...
    """
    db_demand = db.query(Demand).filter(
    ...
    if db_demand is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Demand entry not found")
    return db_demand

@router.put("/{origin}/{destination}/{start_time}/{end_time}", response_model=DemandRead)
def update_demand(
    origin: int,
    destination: int,
    start_time: time,
...
@router.delete("/{origin}/{destination}/{start_time}/{end_time}", status_code=status.HTTP_204_NO_CONTENT)
def delete_demand(
    origin: int,
    destination: int,
    start_time: time,
    ...
    try:
        db.delete(db_demand)
        db.commit()
        return {"message": "Demand entry deleted successfully"} # 204 No Content typically doesn't return a body
    except IntegrityError:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
