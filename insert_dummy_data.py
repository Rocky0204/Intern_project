...
def insert_data(db: Session):
    ...
    service1 = Service(service_code="SER001", name="Morning Express", description="Morning peak service", operator_id=op1.operator_id, line_id=line1.line_id)
    db.add(service1)
    db.flush()

    # 10. JourneyPattern (requires route_id, service_id, line_id, operator_id)
    jp1 = JourneyPattern(jp_code="JP001", name="Central Loop AM", route_id=route1.route_id, service_id=service1.service_id, line_id=line1.line_id, operator_id=op1.operator_id)
    db.add(jp1)
    db.flush()

    # 11. Bus (requires bus_type_id, garage_id, operator_id)
    bus1 = Bus(bus_id="B001", reg_num="XYZ123", bus_type_id=bus_type_small.type_id, garage_id=garage1.garage_id, operator_id=op1.operator_id)
    bus2 = Bus(bus_id="B002", reg_num="ABC456", bus_type_id=bus_type_large.type_id, garage_id=garage1.garage_id, operator_id=op1.operator_id)
    db.add_all([bus1, bus2])
    db.flush()

    # 12. Block (requires operator_id, bus_type_id)
    block1 = Block(name="Morning Block", operator_id=op1.operator_id, bus_type_id=bus_type_small.type_id)
    block2 = Block(name="Evening Block", operator_id=op1.operator_id, bus_type_id=bus_type_large.type_id)
    db.add_all([block1, block2])
    db.flush()

    # 13. Demand (between StopAreas)
    demand1 = Demand(origin=sa1.stop_area_code, destination=sa3.stop_area_code, count=50.0, start_time=time(8, 0), end_time=time(9, 0))
    demand2 = Demand(origin=sa3.stop_area_code, destination=sa1.stop_area_code, count=70.0, start_time=time(17, 0), end_time=time(18, 0))
    demand3 = Demand(origin=sa1.stop_area_code, destination=sa2.stop_area_code, count=20.0, start_time=time(9, 0), end_time=time(10, 0))
