...
from api.models import Base, StopArea, StopPoint, Operator, Line, Service, Bus, BusType, Route, JourneyPattern, VehicleJourney, Block, Demand, EmulatorLog, JourneyPatternDefinition, RouteDefinition, StopActivity, Garage
from api.schemas import StopAreaCreate, StopAreaRead, StopAreaUpdate
...
def test_create_stop_area_duplicate_admin_area_code(client: TestClient, test_stop_area: StopArea):
    duplicate_data = {
        "stop_area_code": 1003,
...
def test_update_stop_area(client: TestClient, db_session: Session, test_stop_area: StopArea):
    update_data = {
        "name": "Updated Central Station",
        "is_terminal": False
    }
    response = client.put(f"/stop_areas/{test_stop_area.stop_area_code}", json=update_data)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    ...
    # Verify in DB
    db_sa = db_session.query(StopArea).filter(StopArea.stop_area_code == test_stop_area.stop_area_code).first()
    assert db_sa.name == update_data["name"]
    assert db_sa.is_terminal == update_data["is_terminal"]

def test_update_stop_area_duplicate_admin_area_code(client: TestClient, db_session: Session):
    # Create a second stop area
    sa2 = StopArea(stop_area_code=1003, admin_area_code="ADM003", name="Temp Area", is_terminal=False)
    db_session.add(sa2)
    db_session.commit()
    db_session.refresh(sa2)

    # Try to update test_stop_area with sa2's admin_area_code
    test_sa = db_session.query(StopArea).filter(StopArea.stop_area_code == 1001).first() # Assuming 1001 is the ID of test_stop_area
    if not test_sa: # If test_stop_area was not created by a fixture, create it here
        test_sa = StopArea(stop_area_code=1001, admin_area_code="ADM001", name="Central Bus Station", is_terminal=True)
        db_session.add(test_sa)
        db_session.commit()
        db_session.refresh(test_sa)

    update_data = {"admin_area_code": sa2.admin_area_code} # Try to use existing admin_area_code
    response = client.put(f"/stop_areas/{test_sa.stop_area_code}", json=update_data)
    assert response.status_code == status.HTTP_409_CONFLICT
    assert "already exists" in response.json()["detail"]


def test_delete_stop_area(client: TestClient, db_session: Session, test_stop_area: StopArea):
    stop_area_code_to_delete = test_stop_area.stop_area_code
    response = client.delete(f"/stop_areas/{stop_area_code_to_delete}")
    assert response.status_code == status.HTTP_204_NO_CONTENT # 204 No Content for successful deletion

    # Verify in DB
    db_sa = db_session.query(StopArea).filter(StopArea.stop_area_code == stop_area_code_to_delete).first()
