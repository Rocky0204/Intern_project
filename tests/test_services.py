...
def test_create_service(
    client: TestClient, db_session: Session, test_operator: Operator, test_line: Line
):
    service_data = {
        "service_code": "S2",
        "name": "New Service",
...
def test_update_service(client: TestClient, db_session: Session, test_service: Service):
    ...
    db_service = (
        ...
        .filter(Service.service_id == test_service.service_id)
        .first()
    )
...
def test_delete_service(client: TestClient, db_session: Session, test_service: Service):
    ...
    db_service = (
        db_session.query(Service)
        .filter(Service.service_id == service_id_to_delete)
        .first()
    )
...
def test_create_service_with_invalid_operator(
    client: TestClient, db_session: Session, test_line: Line
):
    invalid_data = {
        "service_code": "S3",
...
def test_create_service_with_invalid_line(
