...
def _populate_db_for_simulation(db: Session):
    ...
    bus = Bus(bus_id="bus1", reg_num="REG123", bus_type_id=1, garage_id=1, operator_id=1)
    
    db.add_all([operator, garage, bus_type, bus])
    db.commit()

    stop_area1 = StopArea(stop_area_code=1, admin_area_code="ADM1", name="Area 1", is_terminal=True) # is_terminal=True for depot
    stop_area2 = StopArea(stop_area_code=2, admin_area_code="ADM2", name="Area 2")
    db.add_all([stop_area1, stop_area2])
    db.commit()

    stop1 = StopPoint(atco_code=1001, name="Stop A", latitude=51.5, longitude=0.0, stop_area_code=1)
    stop2 = StopPoint(atco_code=1002, name="Stop B", latitude=51.6, longitude=0.1, stop_area_code=1)
...
def test_update_emulator_log_and_run_simulation_patch_exception(
    ...
    assert "Test error during simulation" in updated_db_log.optimization_details_dict["message"]
...
def test_read_emulator_logs(client_with_db: TestClient, test_db_session: Session):
    ...
    log1 = EmulatorLog(status=RunStatus.COMPLETED.value, started_at=datetime.now(timezone.utc), last_updated=datetime.now(timezone.utc))
    log2 = EmulatorLog(status=RunStatus.FAILED.value, started_at=datetime.now(timezone.utc), last_updated=datetime.now(timezone.utc))
...
def test_read_emulator_log_by_id(client_with_db: TestClient, test_db_session: Session):
    ...
    log = EmulatorLog(status=RunStatus.RUNNING.value, started_at=datetime.now(timezone.utc), last_updated=datetime.now(timezone.utc))
...
def test_update_emulator_log_status(client_with_db: TestClient, test_db_session: Session):
    ...
    log = EmulatorLog(status=RunStatus.QUEUED.value, started_at=datetime.now(timezone.utc), last_updated=datetime.now(timezone.utc))
    test_db_session.add(log)
    test_db_session.commit()
    test_db_session.refresh(log)

    response = client_with_db.put(
        f"/emulator_logs/{log.run_id}",
        json={"status": RunStatus.COMPLETED.value}
    )
    assert response.status_code == 200
    assert response.json()["status"] == RunStatus.COMPLETED.value

    updated_log = test_db_session.query(EmulatorLog).filter(EmulatorLog.run_id == log.run_id).first()
...
def test_update_emulator_log_optimization_details(client_with_db: TestClient, test_db_session: Session):
    ...
    response = client_with_db.put(
        f"/emulator_logs/{log.run_id}",
        json={"optimization_details": new_details}
    )
    assert response.status_code == 200
...
def test_delete_emulator_log(client_with_db: TestClient, test_db_session: Session):
    ...
    log = EmulatorLog(status=RunStatus.COMPLETED.value, started_at=datetime.now(timezone.utc), last_updated=datetime.now(timezone.utc))
    test_db_session.add(log)
    test_db_session.commit()
    test_db_session.refresh(log)
