...
from api.schemas import JourneyPatternCreate
from api.database import *

# Import the client_with_db and db_session fixtures from conftest.py
from .conftest import client_with_db, db_session # Removed TestingSessionLocal import as it's not needed directly
...
def test_create_journey_pattern(client_with_db: TestClient, db_session: Session):
    ...
    db_jp = db_session.query(JourneyPattern).filter(JourneyPattern.jp_code == test_data["jp_code"]).first()
...
def test_update_journey_pattern(client_with_db: TestClient, db_session: Session):
    ...
    updated_db_jp = db_session.query(JourneyPattern).filter(JourneyPattern.jp_id == jp_id).first()
...
def test_delete_journey_pattern(client_with_db: TestClient, db_session: Session):
    ...
    deleted_db_jp = db_session.query(JourneyPattern).filter(JourneyPattern.jp_id == jp_id).first()
