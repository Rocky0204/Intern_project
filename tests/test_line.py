...
from api.schemas import LineCreate, LineRead, LineUpdate, RunStatus
...
def test_create_line_duplicate_name(client: TestClient, db_session: Session, test_operator: Operator):
    ...
    initial_line = Line(line_name="Duplicate Line Name", operator_id=test_operator.operator_id)
    ...
    assert "Could not create line due to a database integrity issue (e.g., duplicate line_name)." in response.json()["detail"]
