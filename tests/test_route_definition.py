...
from api.models import Route, StopPoint, RouteDefinition, StopArea # Import necessary models
...
def setup_parent_entities(db_session: Session, index: int = 0): # Added index parameter
    ...
    return db_route.route_id, db_stop_point.atco_code # Return route_id and stop_point_id
...
def test_create_route_definition(client_with_db: TestClient, db_session: Session):
    ...
    """
    route_id, stop_point_atco_code = setup_parent_entities(db_session, index=0) # Pass index
    ...
    db_def = db_session.query(RouteDefinition).filter(
        RouteDefinition.route_id == route_id,
        RouteDefinition.sequence == 1
    ).first()
...
def test_read_route_definitions(client_with_db: TestClient, db_session: Session):
    ...
    """
    route_id_1, stop_point_atco_code_1 = setup_parent_entities(db_session, index=1) # Pass index 1
    route_id_2, stop_point_atco_code_2 = setup_parent_entities(db_session, index=2) # Pass index 2

    # Create definitions for route_id_1 using 'stop_point_id' for direct DB insertion
    def_data_1_1 = {"route_id": route_id_1, "stop_point_id": stop_point_atco_code_1, "sequence": 1}
    def_data_1_2 = {"route_id": route_id_1, "stop_point_id": stop_point_atco_code_1 + 1, "sequence": 2} # Use a different stop point for sequence 2
    ...
    def_data_2_1 = {"route_id": route_id_2, "stop_point_id": stop_point_atco_code_2, "sequence": 1}
...
def test_update_route_definition(client_with_db: TestClient, db_session: Session):
    ...
    Handles the 'stop_point_atco_code' vs 'stop_point_id' mapping.
    """
    route_id, stop_point_atco_code = setup_parent_entities(db_session, index=3) # Pass index
    ...
    update_data_api = {"stop_point_atco_code": stop_point_atco_code + 10} # Use atco_code for API
    response = client_with_db.put(
        f"/route_definitions/{route_id}/{sequence}",
    ...
    updated_db_def = db_session.query(RouteDefinition).filter(
        RouteDefinition.route_id == route_id,
...
def test_delete_route_definition(client_with_db: TestClient, db_session: Session):
    ...
    """
    route_id, stop_point_atco_code = setup_parent_entities(db_session, index=4) # Pass index
    ...
    deleted_db_def = db_session.query(RouteDefinition).filter(
        RouteDefinition.route_id == route_id,
        RouteDefinition.sequence == sequence
    ).first()
    ...
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 0 # No definitions should be found for this route_id after deletion
