...
def test_create_journey_pattern_definition(
    ...
    assert (
        data["stop_point_atco_code"] == test_data_api["stop_point_atco_code"]
    )  # Assert on atco_code
...
def test_read_journey_pattern_definitions(
    ...
    def_data_1_1 = {
        ...
        "departure_time": time(9, 1, 0),
    }
    def_data_1_2 = {
        "jp_id": jp_id_1,
        "stop_point_id": 102,
        "sequence": 2,
        "arrival_time": time(9, 2, 0),
        "departure_time": time(9, 3, 0),
    }
    ...
    response_filtered = client_with_db.get(
        f"/journey_pattern_definitions/?jp_id={jp_id_1}"
    )
...
def test_update_journey_pattern_definition(
    ...
    update_data_api = {
        ...
        "stop_point_atco_code": 2002,
    }  # Use atco_code for API
    response = client_with_db.put(
        f"/journey_pattern_definitions/{jp_id}/{sequence}", json=update_data_api
    )
    assert response.status_code == 200
    data = response.json()
    ...
    assert (
        data["stop_point_atco_code"] == update_data_api["stop_point_atco_code"]
    )  # Assert on atco_code
    ...
    assert updated_db_def.arrival_time == time.fromisoformat(
        update_data_api["arrival_time"]
    )
    assert (
...
def test_delete_journey_pattern_definition(
    ...
    assert (
        len(data) == 0
    )  # No definitions should be found for this jp_id after deletion
